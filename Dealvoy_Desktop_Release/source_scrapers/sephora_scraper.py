...
class SephoraScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Sephora",
            base_url="https://www.sephora.com"
        )
        self.search_endpoint = "https://www.sephora.com/shop/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Sephora scraping not allowed by robots.txt")
            return products
        search_url = f"{self.search_endpoint}?keyword={quote(query)}"
        ...
        if response.status_code != 200:
            print(f"❌ Sephora search failed: {response.status_code}")
            return products
        soup = BeautifulSoup(response.content, 'html.parser')
        product_cards = soup.find_all('div', class_=re.compile(r'css-12egk0t'))
        for card in product_cards[:max_results]:
            title_elem = card.find('span', class_=re.compile(r'css-0'))
            price_elem = card.find('span', class_=re.compile(r'css-0'))
            url_elem = card.find('a', href=True)
            image_elem = card.find('img', class_=re.compile(r'css-0'))
