...
class AceHardwareScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Ace Hardware",
            base_url="https://www.acehardware.com"
        )
        self.search_endpoint = "https://www.acehardware.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Ace Hardware scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Ace Hardware search failed: {response.status_code}")
            return products
