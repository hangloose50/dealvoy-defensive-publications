...
class UlineScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Uline",
            base_url="https://www.uline.com"
        )
        self.search_endpoint = "https://www.uline.com/Search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Uline scraping not allowed by robots.txt")
            return products
        search_url = f"{self.search_endpoint}?keywords={quote(query)}"
        ...
        if response.status_code != 200:
            print(f"❌ Uline search failed: {response.status_code}")
            return products
        soup = BeautifulSoup(response.content, 'html.parser')
        product_cards = soup.find_all('div', class_=re.compile(r'product-listing-item'))
