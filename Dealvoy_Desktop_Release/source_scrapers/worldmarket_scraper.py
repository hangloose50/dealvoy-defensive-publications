def scrape_worldmarket(query: str = "coffee", max_results: int = 10) -> list:
    return WorldMarketScraper().search_products(query, max_results)
...
class WorldMarketScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Cost Plus World Market",
            base_url="https://www.worldmarket.com"
        )
        self.search_endpoint = "https://www.worldmarket.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ World Market scraping not allowed by robots.txt")
            return products
        search_url = f"{self.search_endpoint}?q={quote(query)}"
        ...
        if response.status_code != 200:
            print(f"❌ World Market search failed: {response.status_code}")
            return products
