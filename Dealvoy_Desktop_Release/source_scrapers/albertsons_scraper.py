...
class AlbertsonsScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Albertsons",
            base_url="https://www.albertsons.com"
        )
        self.search_endpoint = "https://www.albertsons.com/shop/search-results.html"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Albertsons scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Albertsons search failed: {response.status_code}")
            return products
