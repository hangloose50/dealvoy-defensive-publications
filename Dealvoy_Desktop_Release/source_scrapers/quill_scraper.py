...
class QuillScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Quill",
            base_url="https://www.quill.com"
        )
        self.search_endpoint = "https://www.quill.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Quill scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Quill search failed: {response.status_code}")
            return products
