...
class BoxedScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Boxed",
            base_url="https://www.boxed.com"
        )
        self.search_endpoint = "https://www.boxed.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Boxed scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Boxed search failed: {response.status_code}")
            return products
