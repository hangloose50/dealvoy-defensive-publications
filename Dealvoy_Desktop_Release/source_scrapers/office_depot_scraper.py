...
class OfficeDepotScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Office Depot",
            base_url="https://www.officedepot.com"
        )
        self.search_endpoint = "https://www.officedepot.com/a/products/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Office Depot scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Office Depot search failed: {response.status_code}")
            return products
