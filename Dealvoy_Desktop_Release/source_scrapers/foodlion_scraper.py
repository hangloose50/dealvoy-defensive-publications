def scrape_foodlion(query: str = "milk", max_results: int = 10) -> list:
    return FoodLionScraper().search_products(query, max_results)
...
class FoodLionScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Food Lion",
            base_url="https://www.foodlion.com"
        )
        self.search_endpoint = "https://www.foodlion.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Food Lion scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Food Lion search failed: {response.status_code}")
            return products
