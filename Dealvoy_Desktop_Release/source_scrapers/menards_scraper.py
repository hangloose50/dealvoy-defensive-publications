...
class MenardsScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Menards",
            base_url="https://www.menards.com"
        )
        self.search_endpoint = "https://www.menards.com/main/search.html"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Menards scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Menards search failed: {response.status_code}")
            return products
