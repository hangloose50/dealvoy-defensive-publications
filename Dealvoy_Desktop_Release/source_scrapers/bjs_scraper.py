...
class BJsScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="BJ's Wholesale Club",
            base_url="https://www.bjs.com"
        )
        self.search_endpoint = "https://www.bjs.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ BJ's scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ BJ's search failed: {response.status_code}")
            return products
