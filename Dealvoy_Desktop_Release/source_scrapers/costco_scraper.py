...
class CostcoScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Costco",
            base_url="https://www.costco.com"
        )
        self.search_endpoint = "https://www.costco.com/CatalogSearch"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Costco scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Costco search failed: {response.status_code}")
            return products
        soup = BeautifulSoup(response.content, 'html.parser')
        product_cards = soup.find_all('div', class_=re.compile(r'product-tile-set'))
        for card in product_cards[:max_results]:
            title_elem = card.find('div', class_=re.compile(r'description'))
