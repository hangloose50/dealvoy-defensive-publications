...
class BHScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="B&H",
            base_url="https://www.bhphotovideo.com"
        )
        self.search_endpoint = "https://www.bhphotovideo.com/c/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ B&H scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ B&H search failed: {response.status_code}")
            return products
        soup = BeautifulSoup(response.content, 'html.parser')
        product_cards = soup.find_all('div', class_=re.compile(r'itemWrapper'))
        for card in product_cards[:max_results]:
            title_elem = card.find('span', class_=re.compile(r'descriptiveText'))
            price_elem = card.find('span', class_=re.compile(r'price_'))
            url_elem = card.find('a', class_=re.compile(r'itemLink'))
            image_elem = card.find('img', class_=re.compile(r'productImage'))
