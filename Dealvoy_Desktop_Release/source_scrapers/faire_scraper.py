...
class FaireScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Faire",
            base_url="https://www.faire.com"
        )
        self.search_endpoint = "https://www.faire.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Faire scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Faire search failed: {response.status_code}")
            return products
