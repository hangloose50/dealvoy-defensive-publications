...
class HEBScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="H-E-B",
            base_url="https://www.heb.com"
        )
        self.search_endpoint = "https://www.heb.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ H-E-B scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ H-E-B search failed: {response.status_code}")
            return products
