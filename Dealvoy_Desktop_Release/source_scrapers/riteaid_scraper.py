...
class RiteAidScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Rite Aid",
            base_url="https://www.riteaid.com"
        )
        self.search_endpoint = "https://www.riteaid.com/shop/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Rite Aid scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Rite Aid search failed: {response.status_code}")
            return products
