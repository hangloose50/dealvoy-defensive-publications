def scrape_giant(query: str = "bread", max_results: int = 10) -> list:
    return GiantScraper().search_products(query, max_results)
...
class GiantScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Giant Food",
            base_url="https://giantfood.com"
        )
        self.search_endpoint = "https://giantfood.com/sm/planning/rsid/1600/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Giant Food scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Giant Food search failed: {response.status_code}")
            return products
