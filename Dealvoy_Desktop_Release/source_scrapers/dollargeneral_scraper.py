...
class DollarGeneralScraper(RetailScraperBase):
    def __init__(self):
        super().__init__(
            source_name="Dollar General",
            base_url="https://www.dollargeneral.com"
        )
        self.search_endpoint = "https://www.dollargeneral.com/search"

    def search_products(self, query: str, max_results: int = 10) -> List[ProductData]:
        products = []
        self.reset_session()
        if not self.check_compliance():
            print("❌ Dollar General scraping not allowed by robots.txt")
            return products
        ...
        if response.status_code != 200:
            print(f"❌ Dollar General search failed: {response.status_code}")
            return products
        ...
        for card in product_cards[:max_results]:
            ...
            price_elem = card.find('span', class_=re.compile(r'product-sales-price'))
            url_elem = title_elem
